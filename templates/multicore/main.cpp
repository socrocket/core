...
int sc_main(int argc, char** argv) {
    ...
    mmu_cache *cache[conf_processors];
#if conf_sys_lt_at
    leon3_funclt_trap::Processor_leon3_funclt *processor[conf_processors];
#else
    leon3_funcat_trap::Processor_leon3_funcat *processor[conf_processors];
#endif

    for(int i = 0; i < conf_processors; i++) {
        // CREATE MMU_CACHE
        // ================
        // Always enabled.
        // Needed for basic platform.
        cache[i] = new mmu_cache(
                ...
                gen_unique_name("mmu_cache"),   // name of sysc module
                ...
                i,                              // Id of the AHB master
                false,                          // Power Monitor,
                ambaLayer                       // LT abstraction
        );
        
        // Connecting AHB Master
        mmu_cache[i].ahb(ahbctrl.ahbIN);
        // Connecting Testbench

        // Set clock
        mmu_cache[i].set_clk(LOCAL_CLOCK,SC_NS);
...
#if conf_sys_lt_at
        processor[i] = new leon3_funclt_trap::Processor_leon3_funclt(
            gen_unique_name("leon3"), 
            sc_core::sc_time(1, SC_US)
        );
#else
        processor[i] = new leon3_funcat_trap::Processor_leon3_funcat(
            gen_unique_name("leon3"), 
            sc_core::sc_time(1, SC_US)
        );
#endif
        ...
        processor[i]->instrMem.initSocket(cache[i].icio);
        processor[i]->dataMem.initSocket(cache[i].dcio);
    }
    ...
    // Initialize memory
    uint8_t *sdramData, *promData;
    ExecLoader sdram_loader(sram_app); 
    sdramData = sdram_loader.getProgData();
    ExecLoader prom_loader(prom_app); 
    promData = prom_loader.getProgData();
    for(unsigned int i = 0; i < sdram_loader.getProgDim(); i++) {
       sdram.write(sdram_loader.getDataStart() + i - ((conf_memctrl_ram_addr&conf_memctrl_ram_mask)<<20), sdramData[i]);
    }
    
    for(unsigned int i = 0; i < prom_loader.getProgDim(); i++) {
       rom.write(prom_loader.getDataStart() + i - ((conf_memctrl_prom_addr&conf_memctrl_prom_mask)<<20), promData[i]);
       //ahb_mem.writeByteDBG(prom_loader.getDataStart() + i, execData[i]);
    }

    for(int i = 0; i < conf_processors; i++) {
        processor[i]->PROGRAM_LIMIT = sdram_loader.getProgDim() + sdram_loader.getDataStart();
        processor[i]->PROGRAM_START = sdram_loader.getDataStart();
        processor[i]->ENTRY_POINT   = prom_loader.getProgStart();
    }
    ...
    OSEmulator<uint32_t> *osEmu[conf_processors];
    for(int p = 0; p < conf_processors; p++) {
        // * OS Emulator ****************************
        // OS Emulator is activating the leon traps to map basic io functions to the host system
        // set_brk, open, read, ...
        osEmu[p] = new OSEmulator<uint32_t>(*(processor[p]->abiIf));
        osEmu[p]->initSysCalls(sram_app);

        processor[p]->toolManager.addTool(osEmu);
        //processor[p]->enableHistory("cmdHistLTSys"); 
        // ******************************************
       
        // * GDBStubs *******************************
        if(gdb_en) {
            GDBStub<uint32_t> *gdbStub = new GDBStub<uint32_t>(*(processor[p]->abiIf));
            processor[p]->toolManager.addTool(*gdbStub);
            gdbStub->initialize(1500 + p); 
            processor[p]->instrMem.setDebugger(gdbStub);
            processor[p]->dataMem.setDebugger(gdbStub);
        }
        // ******************************************
    }
