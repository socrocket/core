...
def swig_c(self):
  ...
  if'-c++'in flags:
    ...
    #c_tsk=self.generator.create_compiled_task('cxx', out_node)
    out='%s.%d.o'%(out_node.name,self.generator.idx)
    c_tsk=self.generator.create_task('cxx',out_node,out_node.parent.find_or_declare(out))
  else:
    #c_tsk=self.generator.c_hook(out_node)
    #c_tsk=self.generator.create_compiled_task('c', out_node)
    out='%s.%d.o'%(out_node.name,self.generator.idx)
    c_tsk=self.generator.create_task('c',out_node,out_node.parent.find_or_declare(out))
  try:
    self.generator.compiled_tasks.append(c_tsk)
  except AttributeError:
    self.generator.compiled_tasks=[c_tsk]
