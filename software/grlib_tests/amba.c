...
int find_ahb_slv(int id, struct ambadev *dev) {
            ...
            for(j = 0; j < 4; j++) {
                switch(ahbpp[i].mem[0] & 0x0F) {
                    case 2:
                        ...
                        ahbdevpp->end[j] = ahbdevpp->start[j] + ((~(ahbpp[i].mem[j] << 16)) + 0x100000) & PPAHBMASK;
                        break;
                    case 3:
                        ahbdevpp->start[j] = ((int) ahbpp) + (ahbpp[i].mem[j] & PPAHBMASK) >> 12;
                        ahbdevpp->end[j] = ahbdevpp->start[j] + (((~(ahbpp[i].mem[j] << 16)) + 0x100000) & PPAHBMASK) >> 12;
                        break;
                }
            }
            break;
