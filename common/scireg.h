...
{
public:
  scireg_region_if() { scireg_tool_registry::add_region(*this); }

  virtual ~scireg_region_if() { scireg_tool_registry::remove_region(*this); }

  // Get the region_type of this region:
  virtual scireg_response scireg_get_region_type(scireg_region_type& t) const { return SCIREG_UNSUPPORTED; }

  // Read a vector of "size" bytes at given offset in this region:
  virtual scireg_response scireg_read(vector_byte& v, sc_dt::uint64 size, sc_dt::uint64 offset=0) const { return SCIREG_UNSUPPORTED; }

  // Write a vector of "size" bytes at given offset in this region:
  virtual scireg_response scireg_write(const vector_byte& v, sc_dt::uint64 size, sc_dt::uint64 offset=0) { return SCIREG_UNSUPPORTED; }
  ...
  virtual sc_dt::uint64 scireg_get_bit_width() const {  return 0; }
  virtual sc_dt::uint64 scireg_get_byte_width() const { return (7 + scireg_get_bit_width()) / 8; }

  // If this region is a register field, these functions return low bit and high bit positions:
  virtual sc_dt::uint64 scireg_get_low_pos() const { return 0; }
  virtual sc_dt::uint64 scireg_get_high_pos() const { return 0; }
  ...
  virtual scireg_response scireg_get_parent_regions(std::vector<scireg_region_if*>& v) const { return SCIREG_UNSUPPORTED; }

  // Add/Delete Callback objects associated with this region
  virtual scireg_response scireg_add_callback(scireg_callback & cb) { return SCIREG_UNSUPPORTED; }
  virtual scireg_response scireg_remove_callback(scireg_callback & cb) { return SCIREG_UNSUPPORTED; }

  // Get SC TLM2 Target socket associated with this region
  virtual scireg_response scireg_get_target_sockets(std::vector<sc_core::sc_object*>& v) const { return SCIREG_UNSUPPORTED; }

  // Get parent SystemC modules associated with this region
  virtual scireg_response scireg_get_parent_modules(std::vector<sc_core::sc_module*>& v) const { return SCIREG_UNSUPPORTED; }

  // Get a vector of value_info objects for this region
  virtual scireg_response scireg_get_value_info(std::vector<scireg_value_info>& v) const { return SCIREG_UNSUPPORTED; }

  // Query to see if DMI access has been granted to this region. "size" and offset can be used to constrain the range.
  virtual scireg_response scireg_get_dmi_granted(bool& granted, sc_dt::uint64 size, sc_dt::uint64 offset=0) const { return SCIREG_UNSUPPORTED; }
};
