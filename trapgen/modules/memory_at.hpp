...
class MemoryAT : public sc_module {
  public:
  MemoryAT(sc_module_name name, unsigned size, sc_time latency = SC_ZERO_TIME) :
    sc_module(name), size(size), latency(latency), trans_id(0),
    trans_in_progress(false), m_peq(this, &MemoryAT::peq_cb) {
    for (int i = 0; i < NUM_INITIATORS; i++) {
      ...
      this->target_socket[i]->register_nb_transport_fw(this, &MemoryAT::nb_transport_fw, i);
      this->target_socket[i]->register_transport_dbg(this, &MemoryAT::transport_dbg, i);
    }
  ...
  tlm::tlm_phase& phase, sc_time& delay) {
    ...
    if (adr > this->size) {
      ...
      std::cerr << "Cannot request address " << std::showbase << std::hex << adr << std::dec << ".\n";
      return tlm::TLM_COMPLETED;
    }
  ...
  void peq_cb(tlm::tlm_generic_payload& trans, const tlm::tlm_phase& phase) {
    ...
    case tlm::END_RESP:
      //std::cerr << "tlm::END_RESP in memory peq_c.\n";
  ...
  public:
  tlm_utils::simple_target_socket_tagged<MemoryAT, sock_size>* target_socket[NUM_INITIATORS];
  private:
  unsigned char* mem;
