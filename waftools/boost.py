...
def set_options(opt):
	...
	opt.add_option('--boost-includes',type='string',default=environ.get("BOOST_DIR",""),dest='boostincludes',help='path to the boost directory where the includes are e.g. /usr/local/include/boost-1_35')
	opt.add_option('--boost-libs',type='string',default=environ.get("BOOST_LIB",""),dest='boostlibs',help='path to the directory where the boost libs are e.g. /usr/local/lib')
