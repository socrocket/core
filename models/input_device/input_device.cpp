...
// Method:
//
// Modified on $Date: 2012-03-15 15:50:45 +0100 (Thu, 15 Mar 2012) $
//          at $Revision: 811 $
//          by $Author: HWSWSIM $
//
...
input_device::input_device(sc_core::sc_module_name name,
    ...
    ahb("ahb_socket", amba::amba_AHB, ambaLayer, false),
    irq("irq"),
    m_framesize(framesize),
    m_frameaddr(frameaddr),
    m_interval(interval),
    m_master_id(hindex), 
    ...
    // Register thread for generating the data frame
    SC_THREAD(gen_frame);
    sensitive << new_frame;

    // Allocate data frame buffer
...
void input_device::frame_trigger() {
  ...
  while(1) {
    ...
    // Start the gen_frame thread every m_interval
    wait(m_interval);
    new_frame.notify();
...
// Generate and transmit a frame of random data
void input_device::gen_frame() {
  ...
  while(1) {
    ...
    for (uint32_t i=0;i<m_framesize;i++) {
      ...
      // AHB write
      mem_write((m_frameaddr << 20)+ (i<<2), 0x8, (unsigned char*)&tmp, 4, &delay, &debug, false);

    }

    v::info << name() << "Transmission of frame completed" << v::endl;

    // Notify CPU by sending interrupt
    irq.write(true);
    wait(clock_cycle);
    irq.write(false);
