...
void AHBCtrl::arbitrate() {
  ...
  connection.master_id = 0;
  connection.slave_id = 0;
  connection.start_time = SC_ZERO_TIME;
  ...
  while(true) {
    ...
    // RTL: A bus master uses the HBUSREQx signal to request access to the bus and may request the bus during any cycle.
    // TLM: Each master can send BEGIN_REQ at any time. There can be multiple requests in parallel. All of them are
    //      entered in the request_map array.
    
    // RTL: The arbiter will sample the request on the rising edge of the clock and then use an internal priority algorithm to
    //      decide which master will be the next to gain access to the bus.
    //      Normally the arbiter will only grant a differnt bus master when a burst is completing.
    // TLM: 
    ...
    if ((address_bus_owner == -1) && ((data_bus_state == RESPONSE) || (data_bus_state == IDLE))) {
      ...
      if (mrrobin == 0) {
        ...
        // Master with the lowest ID has the highest priority
        for(uint32_t i=0; i<16; i++) {

          if (request_map[i].state == PENDING) {

            address_bus_owner = i;
            connection = request_map[i];
            connection.state = SCHEDULED;
            break;
      ...
      // Round-robin
        ...
        for(uint32_t i=0; i<num_of_master_bindings; i++) {
          ...
          if (request_map[robin].state == PENDING) {
      ...
      if (address_bus_owner != -1) {
        ...
        v::debug << name() << "Transaction 0x" << hex << connection.trans << " call to nb_transport_fw with phase " << phase << v::endl;

        // Forward arrow for msc
        msclogger::forward(this, &ahbOUT, connection.trans, phase, delay, connection.slave_id);

        status = ahbOUT[connection.slave_id]->nb_transport_fw(*connection.trans, phase, delay);
        assert((status==tlm::TLM_ACCEPTED)||(status==tlm::TLM_UPDATED));

        if (phase == tlm::END_REQ) {

          m_RequestPEQ.notify(*connection.trans, delay);
          delay = SC_ZERO_TIME;
...
void AHBCtrl::AcceptThread() {
  ...
  while(true) {
    ...
    v::debug << name() << "AHBCTRL4" << v::endl;

    // Get new transaction from AcceptPEQ (nb_transport_fw)
    trans = m_AcceptPEQ.get_next_transaction();

    v::debug << name() << "AHBCTRL5" << v::endl;

    // Extract master id from payload
    amba::amba_id * master_id;
    trans->get_extension<amba::amba_id>(master_id);

    // Find the target (slave)
    int slave_id = get_index(trans->get_address());

    v::debug << name() << "Decoder: slave " << dec << slave_id << " address 0x" << hex << trans->get_address() << v::endl;
    ...
    if (slave_id >= 0) {
      ...
      response_map[slave_id] = connection;
...
void AHBCtrl::EndResponseThread() {
  ...
  while(true) {
    ...
    status = ahbOUT[connection.master_id]->nb_transport_fw(*trans, phase, delay);
