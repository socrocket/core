...
class MEMDevice {
    public:
        ...
        MEMDevice(device_type type, uint32_t bank = 0, uint32_t size = 0, uint32_t bits = 32);
        ...
        /// Returns the bank of the memory (sram or sdram bank if needed)
        virtual const uint32_t get_bank() const;

        /// Returns the size of the memory bank.
