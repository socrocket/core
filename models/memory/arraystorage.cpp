...
void ArrayStorage::erase(const uint32_t start, uint32_t end) {
  memset(&data[start], 0, end-start);
}

void ArrayStorage::write_block(const uint32_t addr, uint8_t *ptr, uint32_t len) {
  memcpy(&data[addr], ptr, len);
}

void ArrayStorage::read_block(const uint32_t addr, uint8_t *ptr, uint32_t len) {
