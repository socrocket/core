...
void MapStorage::erase(const uint32_t start, uint32_t end) {
...
void MapStorage::read_block(const uint32_t addr, uint8_t *ptr, uint32_t len) {
  for (size_t i = 0; i < len; i++) {
    ptr[i] = read(addr+i);
  }
}

void MapStorage::write_block(const uint32_t addr, uint8_t *ptr, uint32_t len) {
  for (size_t i = 0; i < len; i++) {
    write(addr+i, ptr[i]);
  }
}
