...
BaseMemory::BaseMemory(implementation_type type, const uint32_t size) {
  reads = 0;
  reads32 = 0;
  writes = 0;
  writes32 = 0;
...
void BaseMemory::erase(uint32_t start, uint32_t end) {
  this->erase_dbg(start, end);
}

void BaseMemory::erase_dbg(uint32_t start, uint32_t end) {
  storage->erase(start, end);
}


void BaseMemory::write_block(const uint32_t addr, uint8_t *data, uint32_t len) {
  writes += len;
  writes32 += ((len-1)>>2) + 1;
  write_block_dbg(addr, data, len);
}

void BaseMemory::write_block_dbg(const uint32_t addr, uint8_t *data, uint32_t len) {
  storage->write_block(addr, data, len);
}

void BaseMemory::read_block(const uint32_t addr, uint8_t *data, uint32_t len) {
  reads += len;
  reads32 += ((len-1)>>2) + 1;
  read_block_dbg(addr, data, len);
}

void BaseMemory::read_block_dbg(const uint32_t addr, uint8_t *data, uint32_t len) {
