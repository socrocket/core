...
void Mctrl_tb::write(uint32_t addr, uint64_t data, uint32_t width, bool apb) {
    ...
    SHOW;
    std::cout << " WRITE " << gp.get_response_string() << ": 0x" << std::hex
...
void Mctrl_tb::run() {
    ...
    std::cout << std::endl << std::endl
            << "--------------------------------------------------"
            << std::endl
            << "--switching widths to: rom-16, sram-16, sdram-64--"
            << std::endl
            << "--------------------------------------------------"
            << std::endl;

    //switch ROM to 16 bit access
    unsigned int temp = read(Mctrl::Mctrl::MCTRL_MCFG1, 4, APB);
    std::cout << std::endl << "old MCFG1 contents: " << std::hex
            << (unsigned int)temp;
    temp &= ~Mctrl::MCTRL_MCFG1_PROM_WIDTH;
    std::cout << std::endl << "intermediate MCFG1 contents: " << std::hex
            << (unsigned int)temp;
    temp |= (Mctrl::MCTRL_MCFG1_PROM_WIDTH & 0x00000100);
    std::cout << std::endl << "new MCFG1 contents: " << std::hex
            << (unsigned int)temp;
    SET (Mctrl::MCTRL_MCFG1, temp, 4, APB);

    //switch SRAM to 16 bit access
    temp = read(Mctrl::MCTRL_MCFG2, 4, APB);
    std::cout << std::endl << "old MCFG2 contents: " << std::hex
            << (unsigned int)temp;
    temp &= ~Mctrl::MCTRL_MCFG2_RAM_WIDTH;
    std::cout << std::endl << "intermediate MCFG2 contents: " << std::hex
            << (unsigned int)temp;
    temp |= (Mctrl::MCTRL_MCFG2_RAM_WIDTH & 0x00000010);

    //switch SDRAM to 64 bit access
    temp |= Mctrl::MCTRL_MCFG2_D64;
    std::cout << std::endl << "new MCFG2 contents: " << std::hex
            << (unsigned int)temp;
    ...
    std::cout << std::endl << std::endl
            << "--------------------------------------------------"
            << std::endl
            << "------- switching widths to: rom-8, sram-8 -------"
            << std::endl
            << "--------------------------------------------------"
            << std::endl;
    ...
    std::cout << std::endl << "old MCFG4 contents: " << std::hex
            << (unsigned int)temp;
    temp |= Mctrl::MCTRL_MCFG4_PMODE & 0x00050000;
    std::cout << std::endl << "modified MCFG4 contents: " << std::hex
            << (unsigned int)temp;
